plot4 <- function() {
  ...
  ## Set mfrow so we can create 4 plots for a single image
  par(mfrow = c(2,2))
  
  ## Create first plot
  plot(subsetted_file[,1], subsetted_file[,2], type = "l", ylab = "Global Active Power", xlab = "")
  
  ## Create second plot
  plot(subsetted_file[,1], subsetted_file[,4], type = "l", ylab = "Voltage", xlab = "datetime")
  ...
  ## Create fourth plot
  plot(subsetted_file[,1], subsetted_file[,3], type = "l", ylab = "Global_reactive_power", xlab = "datetime")
  dev.off()
